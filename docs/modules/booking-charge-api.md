...
- [`bookings.chargeFlow.integration.test.ts`](../../apps/backend/src/modules/bookings/bookings.chargeFlow.integration.test.ts)（Booking Flow Migration，Integration Test）：11 个测试，涵盖 Create 自动建立 FARE Charge（含 0 元/无 actor 情境）、Edit 建立正负 ADDITION、从 0 元 Edit 成有金额时建立原始 Charge 而非 Adjustment、totalAmountCents 不变时不留多余记录、Cancel 同步 `financialStatus = VOIDED`、Detail 附带 `charges` 净额视图。

## 已知限制

- **既有 Booking 建立流程已经整合**（Booking Flow Migration）：`bookings.service.ts` 的 `createBooking`/`updateBooking` 现在会透过 `createBookingChargeWithClient`（这支 API 内部逻辑的可复用核心，不额外开 Transaction/不额外上锁，供已经在自己 Transaction 里的呼叫方直接复用）自动建立/调整 FARE Charge，`totalAmountCents` 永远反映 Booking Charge 的加总，细节见 [booking.md](./booking.md#booking-charge-整合financial-model-v2)。对 Migration **之前**就已经建立、目前还没有任何 BookingCharge 的旧 Booking，第一次透过这支新 API（或透过 Edit Booking）新增 Charge 时，`totalAmountCents` 会被覆写成只反映新 Charge 的加总，不会保留旧的数值——这是预期中的过渡状态。
